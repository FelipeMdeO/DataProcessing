...
void ADXL362::setupOdr() {
	byte temp = SPIreadOneRegister(0x2C);	// read Reg 2C before modifying

	#ifdef ADXL362_DEBUG
	Serial.print(  "Setting filterCntlReg - Reg 2x before = "); 
	Serial.print(temp, BIN); 
#endif

	// Select HBW and ODR
	byte tempwrite = 0b00010000;			// Setup
	//byte tempwrite = 0b00000001;			// Setup
	//byte tempwrite = 0b00000111;			// Setup
	SPIwriteOneRegister(0x2C, tempwrite);  // Write to FILTER_CTL Register
	delay(10);	
...
void ADXL362::setupNoiseReject() {
	byte temp = SPIreadOneRegister(0x2D);	// read Reg 2D before modifying Noise Mode
	...
	// turn on ultra low noise mode
	byte tempwrite = temp | 0b100000;	     // turn on ultra low noise bit in Reg 2D
	SPIwriteOneRegister(0x2D, tempwrite);   // Write to POWER_CTL_REG, noise
	delay(10);	
...
bool ADXL362::getStatusDataReady() {
	// return getRegisterBits(ADXL362_REG_STATUS,ADXL362_DATA_RDY);
	byte value = SPIreadOneRegister(0x0B);
	value = value & (0x01);
	if (value == 0x01) {
